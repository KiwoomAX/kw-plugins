...
| 첫 빌드가 `Prepare secrets` 단계에서 자격증명 `<저장소 이름>-env` 를 찾지 못하고 멈춘다 | AX 팀이 이 저장소의 자격증명을 아직 만들지 않았다 | AX 팀의 등록 회신을 받은 뒤 Build Now 를 다시 누른다. 요청을 안 보냈으면 SKILL.md 의 「AX 팀에 등록 요청 보내기」로 보낸다 |
